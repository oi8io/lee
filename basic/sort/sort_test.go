...
func TestBubbleSort(t *testing.T) {
	...
	tests := []struct {
		...
		{"五个乱序数字", args{[]int{79, 145, 45, 88, 99}}, []int{45, 79, 88, 99, 145}},
		{"五个乱序数字", args{getInputs()}, getOutputs()},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotOut := BubbleSort(tt.args.in); !reflect.DeepEqual(gotOut, tt.wantOut) {
				t.Errorf("MergeSort() = %v, want %v", gotOut, tt.wantOut)
			}
		})
	}
...
func TestSelectSort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotOut := SelectSort(tt.args.in); !reflect.DeepEqual(gotOut, tt.wantOut) {
				t.Errorf("MergeSort() = %v, want %v", gotOut, tt.wantOut)
			}
		})
	}
