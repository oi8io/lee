...
func maximumImportance(n int, roads [][]int) int64 {
	...
	var ncountMap = make(map[int][]int)
	var counts []int
	for i, i2 := range countMap {
		ncountMap[i2] = append(ncountMap[i2], i)
		counts = append(counts, i2)
	}
	sort.Ints(counts)
	var vMap = make(map[int]int64)
	//fmt.Println(counts)

	for i := len(counts) - 1; i >= 0; i-- {
		for _, i3 := range ncountMap[i] {
			vMap[i3] = int64(n)
			n--
		}
	}
	var sum int64 = 0
	for i := 0; i < len(roads); i++ {
		sum += vMap[roads[i][0]] + vMap[roads[i][1]]
	}
	return sum
}
